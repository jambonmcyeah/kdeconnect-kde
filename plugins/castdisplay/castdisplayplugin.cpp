...
{
    NetworkPacket np(PACKAGE_TYPE_CASTDISPLAY, {{"currentProgram", current}});
    sendPacket(np);
}
...
bool CastDisplayPlugin::receivePacket(const NetworkPacket& np)
{
    if (np.has("command")) {
...
void CastDisplayPlugin::fileReceived(const QUrl& url)
{
    ...
    if (!program.isEmpty()) {
        ...
        NetworkPacket np(PACKAGE_TYPE_CASTDISPLAY, {{"programRequest", true}, {"name", mime.name()}});
        sendPacket(np);
